...
int main() {
  string start;
  string words[] = {"buffalo",    "gorilla",       "apple",   "watermelon",
                    "university", "undergraduate", "coconut", "lily",
                    "helicopter", "home"};
  string hints[] = {"a big animal in the bovine family",
                    "primate",
                    "a common fruit",
...
gameBegin:
  ...
  if (start == "e" || start == "E") {
    ...
    cout << "\nWelcome to the spelling game!\nYou are to guess the correct "
            "word while keeping the following rules in mind:"
         << endl;
    cout << " - Enter 'g' to reveal a correct position for a letter\n - Enter "
            "'n' to skip to the next word (you will not gain any points)\n - "
    ...
    for (int i = 0; i < words_size; i++) {
      ...
      if (i + 1 == words_size) {
        cout << "\n"
             << i + 1 << ".) Scrambled Word (This is the last word): "
             << scramble(words[i]) << endl;
      } else {
        cout << "\n"
          ...
          if (guess == "n" || guess == "N") {
            ...
            if (gcl_uses != 0) {
              if (search < words[i].length()) {
                cout << "\nLetter " << search + 1 << ": " << words[i][search]
                     << endl;
                search += 1;
                gcl_uses -= 1;
                goto userChoice;
              } else {
                cout << "\nThe word has been fully revealed. Please type it "
                        "out or skip to continue."
                     << endl;
                goto userChoice;
              }
            } else {
